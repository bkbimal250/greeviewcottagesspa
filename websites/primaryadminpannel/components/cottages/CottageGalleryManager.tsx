...
export interface ExistingCottageImage {
    id: string;
    url: string;
    alt?: string;
    isPrimary?: boolean;
...
export interface NewCottageImage {
    id: string;
    file: File;
    previewUrl: string;
    isPrimary: boolean;
...
interface CottageGalleryManagerProps {
    existingImages?: ExistingCottageImage[];
    onExistingImagesChange?: (
        images: ExistingCottageImage[],
    ) => void;
    onNewImagesChange?: (
        images: NewCottageImage[],
    ) => void;
    onDeleteExisting?: (
        image: ExistingCottageImage,
    ) => void | Promise<void>;
    maxImages?: number;
    maxSizeMb?: number;
    accept?: string;
...
export default function CottageGalleryManager({
    existingImages = [],
    onExistingImagesChange,
    onNewImagesChange,
    onDeleteExisting,
    ...
    error,
    className = "",
}: CottageGalleryManagerProps) {
    const generatedId = useId();
    const inputId = `cottage-gallery-${generatedId}`;
    const inputRef = useRef<HTMLInputElement>(null);

    const [savedImages, setSavedImages] =
        useState<ExistingCottageImage[]>(existingImages);
    ...
    function addFiles(files: File[]) {
        ...
        const nextImages: NewCottageImage[] = [];
            ...
            <div className="flex flex-col gap-3 border-b border-[var(--border)] pb-4 sm:flex-row sm:items-start sm:justify-between">
                <div>
                    ...
                    <p className="mt-1 text-sm text-[var(--muted)]">
                        Upload cottage interior, exterior and room
                        images.
                    </p>
                </div>
